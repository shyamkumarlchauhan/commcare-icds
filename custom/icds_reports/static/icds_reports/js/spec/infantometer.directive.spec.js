...
describe('Infantometer Directive', function () {

    var $scope, $httpBackend, $location, controller, controllermapOrSectorView;

    pageData.registerUrl('icds-ng-template', 'template');
    pageData.registerUrl('infantometer', 'infantometer');
    ...
    beforeEach(inject(function ($rootScope, $compile, _$httpBackend_, _$location_) {
        ...
        $httpBackend.expectGET('infantometer').respond(200, {
        ...
        $httpBackend.flush();
        $scope.$digest();
        controller = compiled.controller('infantometer');
        controller.step = 'map';
    ...
    it('tests supervisor location', function () {
        ...
        $httpBackend.expectGET('infantometer?location_id=test-id').respond(200, {
            report_data: ['report_test_data'],
        });
        controller.init();
        $httpBackend.flush();
    ...
    it('tests template popup', function () {
        ...
        var expected = '<div class="hoverinfo" style="max-width: 200px !important; white-space: normal;"><p>test</p>' +
            '<div>Number of AWCs that reported having an Infantometer: <strong>10</strong></div>' +
            '<div>% of AWCs that reported having an Infantometer: <strong>10.00%</strong></div></div>';
    ...
    it('tests chart tooltip content', function () {
        ...
        var expected = '<p><strong>Jul 2017</strong></p><br/>' +
            '<div>Number of AWCs that reported having an Infantometer: <strong>10</strong></div>' +
            '<div>% of AWCs that reported having an Infantometer: <strong>50.00%</strong></div>';
    ...
    it('tests horizontal chart tooltip content', function () {
        var expected = '<div class="hoverinfo" style="max-width: 200px !important; white-space: normal;">' +
            '<p>Ambah</p>' +
            '<div>Number of AWCs that reported having an Infantometer: <strong>0</strong></div>' +
            '<div>% of AWCs that reported having an Infantometer: <strong>0.00%</strong></div></div>';
