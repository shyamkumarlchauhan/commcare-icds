...
def get_awc_report_thr(config, month, domain, show_test=False, now_date=None):
    ...
    sorted_result_by_name = sorted(list(chain(ccs_image_data, child_image_data)), key=lambda x: x['person_name'] or '')
    sorted_image_data_list = sorted(sorted_result_by_name, key=lambda x: x['submitted_on'])
