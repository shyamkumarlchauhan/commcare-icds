ICDS Extensions for CommCare HQ
===============================
...
3. Add the following to localsettings:
...
4. Add an `'icds-ucr-citus'` entry to `localsettings.REPORTING_DATABASES` pointing at the desired key from
  `settings.DATABASES` where you want the report data tables to live.
  
  Note that this database must be PostgreSQL > v10 and ideally should be CitusDB (details below). 

5. Create a domain named "icds-cas"
